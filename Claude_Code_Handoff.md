...
**7. 设计矩阵审美优选（产品核心审美资产，已沉淀为 Skill）**
   - **痛点**：竖版出图“很丑”，根因是设计矩阵把像素体、蜡笔体、漫画体、胶带贴纸、手绘涂鸦、粉嫩配色等廉价/幼稚组合与高级组合等概率抽取。
   - **修复**：为 `server/design-matrix.js` 七个维度的每个选项新增 `weight` 字段（3=优先 / 2=普通 / 1=降权 / 0=禁用），并把随机抽样改为**加权随机**（A/B/D 用 Efraimidis-Spirakis 加权不放回洗牌，C/E/F/G 用加权随机）。weight=0 的选项默认不进抽样池，除非用户关键词显式约束。
   - **禁用项（weight=0）**：A9 像素体、A17 蜡笔体、A20 漫画体、C15 胶带粘贴、E7 胶带贴纸、E16 手绘涂鸦。
   - **优先项（weight=3）**：书法/黑体/衬线/艺术装饰/飞白字体，黑白/黑金/白金/莫兰迪/赛博/海洋蓝/冰蓝配色，立体/金属/撕纸/霓虹效果，数字编号/撕纸/光晕/光线装饰，高级冷淡/神秘暗黑/未来科技/奢华精致氛围。
   - **沉淀为 Skill**：新建 `SKILL.md`，完整记录审美定位、权重清单与“调审美的标准动作”（看图→调 weight→跑验证→更新 SKILL.md→提交）。这是产品最终售卖的核心审美，后续所有风格调整都应先改 Skill 与权重。
   - **验证脚本**：`node scripts/verify_matrix.mjs`，统计 2000 次抽样分布、确认禁用项零泄漏、批量去重。调整权重后应重跑。
   - 同步在 `server/prompts.js` 的 `skillPrompt` 中加入 AESTHETIC CURATION 段，让 GPT-4o 规划方案时也偏好高级项、回避廉价项。
